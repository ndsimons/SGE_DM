  ...
  for (treatment in c("NC","LPS")) {
      ...
      weight <- 1 ## Weight to place on betas. Typically either a 1 or a 0, where 0 weights all effects equally.
        ...
        #dir.create(paste0(workDir,"figures/gsea/c",cluster))
          ...
          #### RESTRICT THE SET OF ALL GENES TO JUST ONES AFFILIATED WITH A TF
          sampleBetas <- sampleBetas[sampleBetas$Gene %in% geneTFresults$target,]
            ...
            if(sum(output$p_values[,r] < .05) > 0) {
              for (g in which(output$p_values[,r] < .05)) {
                ...
                ggplot() +
                  ...
                  geom_linerange(aes(x = betasPathway$rank, ymax = betasPathway$betas, ymin = 0), lwd = .15) +
                ...
                ggsave(paste0(figDir,"gsea_stickPlot_c",cluster,"_",treatment,"_",pathwayName,"_TFtargetSet.png"), width = 6, height = 4)
              }
            }
          }
      ...
      ggsave(paste0(figDir,"GSEA_results_sigTFs_upVar_c",cluster,"_",variable,"_",treatment,"_TFtargetSet.png"))
      ...
      ggsave(paste0(figDir,"GSEA_results_sigTFs_downVar_c",cluster,"_",variable,"_",treatment,"_TFtargetSet.png"))
      ...
      ggsave(paste0(figDir,"GSEA_results_sigTFs_c",cluster,"_",variable,"_",treatment,"_TFtargetSet.png"))
  }
}
...
#check TF results
subset(uniqTF, TF == tf)
subset(weightMat.c00.aggRec.LPS, TF == tf)

#which genes are significant
subset(geneTFresults, TF == tf & sigDM)$target
sigTargets <- subset(geneTFresults, TF == tf & sigDM)$target

allTargets <- subset(geneTFresults, TF == tf)$target


#check sample Beta data
sampleBetas[,1:2]

for (t in allTargets) {
  print(res_full_aggRec_DMpbRegGE_c00[rownames(res_full_aggRec_DMpbRegGE_c00) == t,])
}


ggplot(res_full_aggRec_DMpbRegGE_c00) +
  geom_point(aes(x = beta_trtLPS.aggRec_cent,
                 y = beta_trtLPS.aggRec_cent / sqrt(sdev_trtLPS.aggRec_cent)),
                 alpha = .1)
```
