...
for (cluster in c("00","02")) {
  for (treatment in c("LPS","NC")) {
      ...
      for (t in uniqTF$TF) {
        ...
        if (t %in% rownames(tmpDMotherGenes)) {
          tmpDMotherGenes <- tmpDMotherGenes[-which(rownames(tmpDMotherGenes) == t),]
        }
        
        if (dim(tmpDMgenes)[1] > 0) {
            
          targetBetas <- tmpDMgenes[,betaColNum]
          otherBetas <- tmpDMotherGenes[,betaColNum]
          
          allBetas <- dmResults[,betaColNum]
          
          ksTestResults <- ks.test(targetBetas,otherBetas, alternative = "less")
      
          uniqTF$KSest[uNum] <- ksTestResults$statistic
          uniqTF$KSpval[uNum] <- ksTestResults$p.value
        
        } else {
          uniqTF$KSest[uNum] <- NA
          uniqTF$KSpval[uNum] <- NA
        }
        
      }
      
      uniqTF$KSsig <- uniqTF$KSpval < .05
      uniqTF$tvalQs <- with(uniqTF, cut(tval,breaks=quantile(tval, probs=seq(0,1, by=0.25), na.rm=TRUE), 
                                      include.lowest=TRUE, labels=c("Q1","Q2","Q3","Q4")))
      
      
      ggplot(uniqTF, aes(x = tval, y = KSest)) +
        geom_point(aes(col = KSsig)) +
        xlab("<---------- Weaker TF in Higher AgRec   |    tval quartile") +
        geom_smooth(method = "lm") + ggtitle("All TF's", subtitle = paste0("c",cluster," ",variable," ",treatment))
      
      ggsave(paste0(figDir,"KS_est_by_TFbeta_xy_allLinks_c",cluster,"_",variable,"_",treatment,".png"))
      
      
      ggplot(uniqTF, aes(x = tval, y = -log10(KSpval))) +
        xlab("<---------- Weaker TF in Higher AgRec   |    tval quartile") +
        geom_point(aes(col = KSsig)) + ggtitle("All TF's")
      
      ggplot(subset(uniqTF, nGenes > 9), aes(x = tval, y = KSest)) +
        geom_point(aes(col = KSsig)) +
        xlab("<---------- Weaker TF in Higher AgRec   |    tval quartile") +
        geom_smooth(method = "lm") + ggtitle("TF's with at least 10 Target Links", subtitle = paste0("c",cluster," ",variable," ",treatment))
      
      ggsave(paste0(figDir,"KS_est_by_TFbeta_xy_10Links_c",cluster,"_",variable,"_",treatment,".png"))
      
      
      
      ggplot(subset(uniqTF, nGenes > 9), aes(x = tval, y = -log10(KSpval))) +
        xlab("<---------- Weaker TF in Higher AgRec   |    tval quartile") +
        geom_point(aes(col = KSsig)) + ggtitle("TF's with at least 10 Target Links")
      
      
      ggplot(uniqTF, aes(x = tvalQs, y = KSest)) +
        geom_violin(aes(group = tvalQs, fill = tvalQs)) + ggtitle("All TF's")
      
      
      ggplot(uniqTF, aes(x = tval, y = KSest)) +
        geom_point(aes(col = KSsig)) +
        geom_smooth(method = "lm") + ggtitle("All TF's")
      
      
      ggplot(uniqTF, aes(x = nGenes, y = KSest)) +
        geom_point(aes(col = KSsig)) +
        geom_smooth(method = "lm")
      
      ggplot(uniqTF, aes(x = nGenes, y = -log10(KSpval))) +
        geom_point(aes(col = KSsig)) +
        geom_smooth(method = "lm")
      
      
      ggplot(uniqTF, aes(x = tvalQs, y = KSest, group = tvalQs)) +
        geom_violin(aes(fill = tvalQs), draw_quantiles = c(.25,.5,.75)) + 
        geom_jitter(alpha = .5, width = .05) + 
        xlab("<---------- Weaker TF in Higher AgRec   |    tval quartile") +
        ggtitle("All TF's", subtitle = paste0("c",cluster," ",variable," ",treatment))
      
      ggsave(paste0(figDir,"KS_est_by_TFbeta_allLinks_c",cluster,"_",variable,"_",treatment,".png"))
      
      ggplot(subset(uniqTF, nGenes > 9), aes(x = tvalQs, y = KSest, group = tvalQs)) +
        geom_violin(aes(fill = tvalQs), draw_quantiles = c(.25,.5,.75)) + 
        geom_jitter(alpha = .5, width = .05) + 
        xlab("<---------- Weaker TF in Higher AgRec   |    tval quartile") +
        ggtitle("TF's with at least 10 Target Links", subtitle = paste0("c",cluster," ",variable," ",treatment))
      
      ggsave(paste0(figDir,"KS_est_by_TFbeta_10links_c",cluster,"_",variable,"_",treatment,".png"))
      
      
      ggplot(data = subset(geneTFresults, nTargets > 9), 
             aes(x = DMbeta / sqrt(DMsdev), 
                       y = TFslope / TFse)) +
        geom_point(aes(col = sigCOMB), alpha = .5) +
        xlab("DM Std Beta             (Higher Var in Higher AgRec ---------->)") +
      ...
      ggsave(paste0(figDir,"TFbeta_v_DMbeta_xy_top10TFs_c",cluster,"_",variable,"_",treatment,".png"))
  }
}
...
for (cluster in c("00","02")) {
  for (treatment in c("LPS","NC")) {
      ...
      #ggsave(paste0(figDir,"imp02/","Number_of_Samples_hist_c",cluster,"_",variable,"_",treatment,".png"))
      ...
      #ggsave(paste0(figDir,"imp02/","Number_of_Targets_hist_c",cluster,"_",variable,"_",treatment,".png"))
      ...
      #ggsave(paste0(figDir,"imp02/","TFbeta_v_DMbeta_xy_allTFs_c",cluster,"_",variable,"_",treatment,".png"))
      ...
      ggplot(uniqTF, aes(x = nGenes, y = tval)) +
        ...
        ggtitle("TF analysis StdBeta v. # of Targets",
                subtitle = paste0("c",cluster," ",variable," ",treatment))
      #ggsave(paste0(figDir,"imp02/","Targets_v_TFbeta_xy_allTFs_c",cluster,"_",variable,"_",treatment,".png"))
      ...
      ggplot(subset(uniqTF, nGenes > 9), aes(x = nGenes, y = tval)) +
        ...
        ggtitle("TF analysis StdBeta v. # of Targets | >=10 target threshold", 
                subtitle = paste0("c",cluster," ",variable," ",treatment))
      #ggsave(paste0(figDir,"imp02/","Targets_v_TFbeta_xy_top10TFs_c",cluster,"_",variable,"_",treatment,".png"))
      ...
      ggplot(geneTFresults, aes(x = nTargets, y = count)) +
        ...
        ggtitle("All Links: # of Samples v. # of Targets", 
                subtitle = paste0("c",cluster," ",variable," ",treatment))
      #ggsave(paste0(figDir,"imp02/","Samples_v_Targets_xy_allTFs_c",cluster,"_",variable,"_",treatment,".png"))
      ...
      #ggsave(paste0(figDir,"imp02/","TFbeta_v_DMbeta_xy_top10TFs_c",cluster,"_",variable,"_",treatment,".png"))
      ...
      ggplot(data = sigPrint) +
        ...
        ggtitle("GSEA Results - Significant, Weak Network-High AgRec TF's",
                subtitle = paste0("Cluster ",cluster," ",treatment," Treatment")) +
        ylab("TF") + xlab("GSEA Enrichment Score")
...
betasOnly <- as.data.frame(tmpDMotherGenes[,betaColNum])
colnames(betasOnly)[1] <- "beta"

betasPathway <- as.data.frame(tmpDMgenes[,betaColNum])
colnames(betasPathway)[1] <- "beta"

betasOnly$target <- FALSE
betasPathway$target <- TRUE

TFbetas <- rbind(betasOnly,betasPathway)

TFbetas$rank <- rank(TFbetas$beta)

ggplot() +
  geom_linerange(data = subset(TFbetas, target), aes(x = rank, ymax = beta, ymin = 0), alpha = 1, lwd = .1) +
  geom_linerange(data = subset(TFbetas, !target), aes(x = rank, ymax = beta, ymin = 0), alpha = .05, lwd = .1) +
  ggtitle(t,subtitle = paste0("KS estimate=",round(uniqTF$KSest[uNum],digits = 3),
                              " p-val=",round(uniqTF$KSpval[uNum],digits = 3),
                              " targets=",uniqTF$nGenes[uNum],
                              " tVal=",round(uniqTF$tval[uNum], digits = 3)))
...
weight <- 1 ## Weight to place on betas. Typically either a 1 or a 0, where 0 weights all effects equally.
  ...
  #dir.create(paste0(workDir,"figures/gsea/c",cluster))
          ...
          ggplot() +
            ...
            xlab("Rank") + ggtitle(pathwayName, 
                                   subtitle = paste0(subset(GO_res, GO == pathwayName)$n_genes," genes, ",
                                                     ...
                                                     " NES = ",round(as.numeric(NESoutput), digits = 3),
                                                     "    ",colnames(output$p_values)[r]))
          ggsave(paste0(figDir,"gsea_stickPlot_c",cluster,"_",pathwayName,".png"), width = 6, height = 4)
        }
      }
    }
    ...
    assign(x = paste0("gseaTFc",cluster,"by",variable), value = printOut)
...
printOut[printOut$pathway %in% sigTFlist,]

ggplot(data = printOut[printOut$pathway %in% sigTFlist,]) +
  geom_point(aes(x = NES_upVar, y = pathway, col = pval_upVar < .05))

ggplot(data = printOut[printOut$pathway %in% sigTFlist,]) +
  geom_point(aes(x = NES_downVar, y = pathway, col = pval_downVar < .05))
```
