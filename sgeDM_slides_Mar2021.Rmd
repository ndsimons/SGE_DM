...
#treatment
#pvalue
betaBinomPvalDF <- read.table(paste0(plotDataDir,"betaBinomTestDataFrame.txt"))

betaBinomPvalDF$cellClusterName <- ifelse(betaBinomPvalDF$cellClustBinom == 0, "CD8",
                                       ifelse(betaBinomPvalDF$cellClustBinom == 14, "CD4",
                                              ifelse(betaBinomPvalDF$cellClustBinom == 2, "NK",
                                                     ifelse(betaBinomPvalDF$cellClustBinom == 5, "Bcells", "CD14hi"))))

betaBinomPvalDF <- subset(betaBinomPvalDF, cellClustBinom != 5)

pValplotTable <- betaBinomPvalDF[,c(2,8,3,5,4,7)]

pValplotTable <- pValplotTable[order(pValplotTable$cellClusterName, decreasing = TRUE),]


# Define the start and end dates for the data range


for (behavPlot in c("groom", "aggRec")) {
  
  if (behavPlot == "groom") {
    titleText <- "Binomial Test Results - Overall Grooming"
  } else {
    titleText <- "Binomial Test Results - Agonisms Received"
  }
  
  gtBehav <- pValplotTable %>%
    filter(behaveVarBinom == behavPlot) %>%
    select(-behaveVarBinom) %>%
    gt()%>%
    tab_header(
      title = titleText,
      subtitle = glue::glue("Cell Clusters with Multiple Significant Genes")
    ) %>% 
    cols_label(
      treatCondBinom = "Treatment",
      cellClusterName = "Cell Type",
      betaDownCount = "- Beta",
      pvalweightBetas = "P-value",
      betaUpCount = "+ Beta"
    ) %>% 
    tab_style(
      style = list(
      ...
      locations = cells_body(
        columns = 5,
        rows = which(subset(pValplotTable, behaveVarBinom == behavPlot)$pvalweightBetas < .05)
      )
    ) %>% 
    tab_style(
      style = list(
        cell_fill(color = "#FDE725"),
        cell_text(weight = "bold")
        ),
      locations = cells_body(
        columns = 1,
        rows = which(subset(pValplotTable, behaveVarBinom == behavPlot)$cellClusterName == "NK")
      )
    ) %>% 
    tab_style(
      style = list(
        cell_fill(color = "#35B779"),
        cell_text(weight = "bold")
        ),
      locations = cells_body(
        columns = 1,
        rows = which(subset(pValplotTable, behaveVarBinom == behavPlot)$cellClusterName == "CD8")
      )
    ) %>% 
    tab_style(
      style = list(
      ...
      locations = cells_body(
        columns = 1,
        rows = which(subset(pValplotTable, behaveVarBinom == behavPlot)$cellClusterName == "CD4")
      )
    ) %>% 
    tab_style(
      style = list(
      ...
      locations = cells_body(
        columns = 1,
        rows = which(subset(pValplotTable, behaveVarBinom == behavPlot)$cellClusterName == "CD14hi")
      )
    )
  
  gtBehav
  
  gtsave(gtBehav, paste0(figDir,"/7_table_sigBetas_allCells_",behavPlot,"_color.png"))
}
...
```{r beta hist with pvalues}
  ...
  combPvalNull <- vector()
  combPvalWeight <- vector()
  combUpCount <- vector()
  combDownCount <- vector()
  cellClustBinomComb <- vector()
  behaveVarBinomComb <- vector()
    ...
    for (cluster in c("00","14","02","05","06","07")) {
      ...
      for (treatment in c("NC","LPS")) {
        ...
        if (sum(posBetas,negBetas) > 0) {
          ...
          #write biniomial test results to vectors
          cellClustBinom <- c(cellClustBinom,cluster)
          behaveVarBinom <- c(behaveVarBinom,variable)
          trtBinom <- c(trtBinom,treatment)
          betaUpCount <- c(betaUpCount,posBetas)
          betaDownCount <- c(betaDownCount,negBetas)
          pvalNull <- c(pvalNull,signif(posBinomNull$p.value, digits = 3))
          pvalweightBetas <- c(pvalweightBetas,signif(posBinomBkg$p.value, digits = 3))
      ...
      ### combined NC and LPS
      combPosRatio <- mean(c(emrml$NCstdBeta,emrml$LPSstdBeta) > 0)
      
      combPosBetas <- sum(get(paste0("plotEmrml",cluster,variable,"LPS"))[,23] > 0) + sum(get(paste0("plotEmrml",cluster,variable,"NC"))[,22] > 0)
      combNegBetas <- sum(get(paste0("plotEmrml",cluster,variable,"LPS"))[,23] < 0) + sum(get(paste0("plotEmrml",cluster,variable,"NC"))[,22] < 0)
      
      if (sum(combPosBetas,combNegBetas) > 0) {
        #binomial test of 50-50 pos/neg betas
        posBinomNullcomb <- binom.test(combPosBetas, sum(combPosBetas,combNegBetas), p = .5)
        
        #binomial test of pos/neg betas from exact distribution
        posBinomBkgcomb <- binom.test(combPosBetas, sum(combPosBetas,combNegBetas), p = combPosRatio)
        
        binomPvalcombNull <- signif(posBinomNullcomb$p.value, digits = 3)
        binomPvalcomb <- signif(posBinomBkgcomb$p.value, digits = 3)
        
        cellClustBinomComb <- c(cellClustBinomComb,cluster)
        behaveVarBinomComb <- c(behaveVarBinomComb,variable)
        combPvalNull <- c(combPvalNull,binomPvalcombNull)
        combPvalWeight <- c(combPvalWeight,binomPvalcomb)
        combUpCount <- c(combUpCount,combPosBetas)
        combDownCount <- c(combDownCount,combNegBetas)
  ...
  betaBinomPvalDF$behaveVarBinom <- behaveVarBinom
  betaBinomPvalDF$trtBinom <- trtBinom
  betaBinomPvalDF$betaUpCount <- betaUpCount
  betaBinomPvalDF$betaDownCount <- betaDownCount
  betaBinomPvalDF$pvalNull <- pvalNull
  betaBinomPvalDF$pvalweightBetas <- pvalweightBetas
  ...
  betaBinomPvalCombDF <- as.data.frame(cellClustBinomComb)
  
  betaBinomPvalCombDF$behaveVarBinom <- behaveVarBinomComb
  betaBinomPvalCombDF$betaUpCount <- combUpCount
  betaBinomPvalCombDF$betaDownCount <- combDownCount
  betaBinomPvalCombDF$pvalNull <- combPvalNull
  betaBinomPvalCombDF$pvalweightBetas <- combPvalWeight
  
  write.table(x = betaBinomPvalCombDF, file = paste0(plotDataDir,"betaBinomCombinedTestDataFrame.txt"))
...
```{r density for poster}

ggplot(betaHistAgRec, aes(x = betaVal, y = cellClusterName, group = cellClusterName, color = cellClusterName, fill = cellClusterName)) +
  geom_density_ridges() + scale_fill_viridis(discrete=TRUE) + theme_minimal() + scale_color_viridis(discrete=TRUE) +
  ylab("") + xlab("Significant Beta Values, Cell-to-Cell Variance")  + 
  theme(legend.position = "none",
        plot.title = element_text(hjust = 0.5, size = 22),
        axis.text.y = element_text(size = 16, vjust = -1),
        axis.text.x = element_text(size = 12),
        axis.title.x = element_text(size = 16)) +
  ggtitle("Agonisms Received")

ggsave(paste0(figDir,"/7_sigBetas_allCells_agRec_hist_poster.png"))
...
ggsave(paste0(figDir,"/7_sigBetas_allCells_groom_hist_poster.png"))
...
paulResults <- readRDS("~/Dropbox (Personal)/scRNA/SGE_DM/paulData/res_fdr_celltype_CD8_A_modelnum_1_covariate_gc.overall.agg.rec_nperms_100.RDS")
...
lpsResults$colorSig <- ifelse(lpsResults$LPSsigDM & lpsResults$LPSsigMean, "#35B779",
                            ifelse(lpsResults$LPSsigDM, "#31688E",
                                   ifelse(lpsResults$LPSsigMean, "#FDE725", "gray60")))



ggplot(data = lpsResults, aes(x = LPSstdBetaMean, y = LPSstdBetaDM)) +
  geom_point(col = lpsResults$colorSig, alpha = .66) +
  geom_smooth(method = "lm", col = "black", alpha = .05) +
  xlab("Mean Effect Standardized Betas") + 
  ylab("DM Effect Standardized Betas") +
  theme_minimal() +
  theme(legend.position = "none") +
...
ggsave(paste0(figDir,"/8_stdBetas_DM_v_mean_CD8A_LPS.png"), width = 6, height = 4)

#genes to check SH2D1A
#
#
...
subset(lpsResults, combSig == "dm" & abs(LPSstdBetaMean) < .5)
subset(lpsResults, combSig == "mean" & abs(LPSstdBetaDM) < .5)

#geneName <- "RHOG"

for (geneName in c("RHOG","PVRL2","HDDC2","HIPK2","SPRYD3","APH1B")) {
  
  #geneName <- "RHOG"
  
  geneNum <- which(rownames(sge_LPS_pbMean_matrix) == geneName)
  
  DMeffectDF <- as.data.frame(unlist(sge_LPS_pbMean_matrix[geneNum,]))
  
  colnames(DMeffectDF) <- "mean"
  
  DMeffectDF$DM <- unlist(sge_LPS_DMpbResid_matrix[geneNum,])
  DMeffectDF$agRec <- subset(metadata, trt == "LPS")$aggRec_cent
  DMeffectDF$groom <- subset(metadata, trt == "LPS")$groom_cent
  ...
  meanAgrec <- ggplot(DMeffectDF, aes(x = agRec, y = mean)) +
    geom_point(col = meanColor, size = 5) + xlab("Agonisms Received") + ylab("Mean") +
    geom_smooth(method = "lm", se = FALSE, col = meanColor, lwd = 3) +
    ggtitle(geneName) +
    theme_minimal() +
  ...
  meanAgrec
  ggsave(paste0(figDir,"6_mean_v_agRec__by_individual_c",cluster,"_",geneName,".png"))
  
  DMAgrec <- ggplot(DMeffectDF, aes(x = agRec, y = DM)) +
    geom_point(col = dmColor, size = 5) + xlab("Agonisms Received") + ylab("DM") +
    geom_smooth(method = "lm", se = FALSE, col = dmColor, lwd = 3) +
    ggtitle(geneName) +
    theme_minimal() +
    theme(plot.title = element_text(size = 22),
          axis.title = element_text(size = 22))
  ...
  DMAgrec
  ggsave(paste0(figDir,"6_DM_v_agRec__by_individual_c",cluster,"_",geneName,".png"))
  
  plot_grid(meanAgrec, DMAgrec, ncol = 2)
  ggsave(paste0(figDir,"6_mean_and_DM_v_agRec__by_individual_c",cluster,"_",geneName,".png"), width = 6, height = 4)
}

subset(lpsResults, gene %in% c("RHOG","PVRL2","HDDC2","HIPK2","SPRYD3","APH1B"))
```

```{r poster slide 10 variance mean relationships}


ind <- 14

varMean <- ggplot(data = rawDataDF, aes(x = mean, y = var)) +
  geom_point(alpha=0.5, col = varColor, size = 3) +
  stat_smooth(method='lm', color='gray40', alpha = .8, fullrange = T) +
  xlab(paste0('Pseudobulk Mean GE')) +
  ylab(paste0('Variance')) +
  theme_minimal() +
      theme(plot.title = element_text(size = 22),
          axis.title = element_text(size = 22))
...
varMean
ggsave(paste0(figDir,"10_resid_var_v_pbMean_ge__by_individual_c02.png"))

dmMean <- ggplot(data = rawDataDF, aes(x = mean, y = DM)) +
  geom_point(alpha=0.5, col = dmColor, size = 3) +
  stat_smooth(method='lm', color='gray40', alpha = .8, fullrange = T) +
  xlab(paste0('Pseudobulk Mean GE')) +
  ylab(paste0('DM')) +
  theme_minimal() +
...
dmMean
ggsave(paste0(figDir,"10_resid_DMpb_v_pbMean_ge__by_individual_c02.png"))


plot_grid(varMean,dmMean, ncol = 2)
ggsave(paste0(figDir,"10_resid_DMpb_and_var_v_pbMean_ge__by_individual_c02.png"))


#cutoff at GE of 4

varMean <- varMean + xlim(c(0,3.5)) + ylim(c(0,40))

varMean
ggsave(paste0(figDir,"10_resid_var_v_pbMean_ge__by_individual_c02_xlim4.png"))

dmMean <- dmMean

dmMean
ggsave(paste0(figDir,"10_resid_DMpb_v_pbMean_ge__by_individual_c02_xlim4.png"))


plot_grid(varMean,dmMean, ncol = 2)
ggsave(paste0(figDir,"10_resid_DMpb_and_var_v_pbMean_ge__by_individual_c02_xlim4.png"))
