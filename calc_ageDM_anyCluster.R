...
workDir <- "/data/tunglab/crc/macaqueRNA/ageDM/"
...
age_pbmc <- readRDS("/data/tunglab/nds/AS_7SEP20.rds")
#sge_pbmc_NC <- subset(sge_pbmc, subset = trt == 'NC')
#sge_pbmc_LPS <- subset(sge_pbmc, subset = trt == 'LPS')
#rm(sge_pbmc)

#what are the cell clusters?
cellClusters <- sort(unique(Idents(age_pbmc)))

for (clusterNum in cellClusters) {
  #clusterNum <- 5
  print(clusterNum)
  cObj <- SubsetData(object = age_pbmc, do.clean = TRUE, do.scale = TRUE, do.center = TRUE, ident.use = clusterNum)
  cMeta <- cObj@meta.data %>% as.data.table
  
  cNumDouble <- paste0(floor(as.numeric(clusterNum)/10),as.numeric(clusterNum) %% 10)
  ...
  for (c in unique(cMeta$ID)) {
      if ( dim(cMeta[ID == c])[1] >= cellThresh ) {
  ...
  for (i in ind_list){
    ...
    tmp <- subset(age_pbmc, subset = ID == i)
    ...
    assign(value = object_pseudobulkMeans, x = paste(i,"_counts_c",cNumDouble,"_pbMean", sep=""))
    ...
    assign(value = tmp2, x = paste(i,"_counts_c",cNumDouble, sep=""))
    ...
    assign(value = DMlevels, x = paste(i,"_counts_c",cNumDouble,"_DM", sep=""))
    assign(value = meanGenes, x = paste(i,"_counts_c",cNumDouble,"_mean", sep=""))
    assign(value = varGenes, x = paste(i,"_counts_c",cNumDouble,"_var", sep=""))
    assign(value = DMpb, x = paste(i,"_counts_c",cNumDouble,"_DMpb", sep=""))
  }
  ...
  for (metric in c("DM","mean","var","pbMean","DMpb")) {
    ...
    for (j in ind_list) {
      tmpOutput <- cbind(tmpOutput,as.matrix(get(paste0(j,"_counts_c",cNumDouble,"_",metric))))
    }
    ...
    colnames(tmpOutput) <- ind_list
    
    
    assign(value = tmpOutput, x = paste0("age_",cNumDouble,"_",metric,"_matrix"))
    
    write.table(tmpOutput,file=paste0(workDir,paste0("age_c",cNumDouble,"_",metric,"_matrix")),row.names=T,col.names=T,quote=F,sep='\t')
  }
}
