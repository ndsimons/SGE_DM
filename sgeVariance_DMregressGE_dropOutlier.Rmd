...
workDir <- "~/Dropbox (Personal)/scRNA/SGE_DM/rGE_dropDV/"
...
plotModelHistsNoFDR <- function(emmremlDF, folderName = "~/Downloads/", modelName = "model") {
...
for (cluster in c("00","14","02","05","06","07")[3]) {
  print(paste0("starting cluster ",cluster))
  #cluster <- "02"
  ...
  ##### REMOVE DV2J FROM THE ANALYSIS - testing the effect of the outlier
  #step 1 - check for DV2J
  if ( "DV2J" %in% colnames(sge_NC_DMpb_matrix) ) {
  sge_LPS_pbMean_matrix <- sge_LPS_pbMean_matrix[,-which(colnames(sge_LPS_pbMean_matrix) == "DV2J")]
  sge_LPS_DMpb_matrix <- sge_LPS_DMpb_matrix[,-which(colnames(sge_LPS_DMpb_matrix) == "DV2J")]
  
  sge_NC_pbMean_matrix <- sge_NC_pbMean_matrix[,-which(colnames(sge_NC_pbMean_matrix) == "DV2J")]
  sge_NC_DMpb_matrix <- sge_NC_DMpb_matrix[,-which(colnames(sge_NC_DMpb_matrix) == "DV2J")]
  }
  ...
  ### Regress mean out of EVERY GENE
  sge_LPS_DMpbResid_matrix <- sge_LPS_DMpb_matrix
  ...
  LPS_DMmeans <- rowMeans(sge_LPS_DMpbResid_matrix)
  NC_DMmeans <- rowMeans(sge_NC_DMpbResid_matrix)
  
  print(paste0("taking residuals"))
  for (g in 1:dim(sge_LPS_DMpb_matrix)[1]) {
    ...
    tmpData <- as.data.frame(cbind(t(sge_LPS_pbMean_matrix[g,]),t(sge_LPS_DMpb_matrix[g,])))
    colnames(tmpData) <- c("mean","DM")
    sge_LPS_DMpbResid_matrix[g,] <- lm(tmpData$DM ~ tmpData$mean)$residuals
  }
  
  #add the average DM for NC and LPS back in
  sge_LPS_DMpbResid_matrix <- sge_LPS_DMpbResid_matrix + LPS_DMmeans
  ...
  #and raw means and variances and residuals
  sge_DMpbResid_matrix_nested <- merge(sge_NC_DMpbResid_matrix,sge_LPS_DMpbResid_matrix, by=0)
  rownames(sge_DMpbResid_matrix_nested) <- sge_DMpbResid_matrix_nested$Row.names
  sge_DMpbResid_matrix_nested$Row.names <- NULL
  ...
  if ( sum(clustResults %in% sgeOutput) == 3 ) {
    ...
    for (f in list.files(path = paste0(workDir,"cluster/permutations/"))[fileID]) {
      ...
      print(paste0("done with cluster ",cluster))
    }
    
  } else {
    #need to run the loop locally
  
    ## regress out group effects from DMpb_residuals ##
    design <- model.matrix(~metadata$group)
    fit <-lmFit(sge_DMpbResid_matrix_nested,design)
    intercepts=data.frame(eBayes(fit))[,1]
    resid_DMpb=apply(residuals.MArrayLM(object=fit, sge_DMpbResid_matrix_nested),2,function(x){x+intercepts})
    ...
    p1 <- ggplot(sge_NC_pbMean_matrix, aes(sge_NC_pbMean_matrix[,1],sge_NC_DMpbResid_matrix[,1])) +
      geom_point(alpha=0.5) +
      stat_smooth(method='lm', color='black', fullrange = T) +
      xlab('mean ge - individual 1') +
      ylab('resid_DMpb - individual 1') +
      theme_minimal()
    p2 <- ggplot(sge_NC_pbMean_matrix, aes(sge_NC_pbMean_matrix[,2],sge_NC_DMpbResid_matrix[,2])) +
      geom_point(alpha=0.5) +
      stat_smooth(method='lm', color='black', fullrange = T) +
      xlab('mean ge - individual 2') +
      ylab('resid_DMpb - individual 2') +
      theme_minimal()
    p3 <- ggplot(sge_NC_pbMean_matrix, aes(sge_NC_pbMean_matrix[,3],sge_NC_DMpbResid_matrix[,3])) +
      geom_point(alpha=0.5) +
      stat_smooth(method='lm', color='black', fullrange = T) +
    ...
    ### modeling DMpb
    ###### Fit an elo model for each gene using emmreml to DM pseudobulk residuals
    ...
    assign(value = data.frame(res_full), x = paste0("res_full_DMpbRegGE_c",cluster))
    
    plotModelHistsNoFDR(get(paste0("res_full_DMpbRegGE_c",cluster)), 
                        folderName = paste0(figDir,"c",cluster,"/"), 
                        modelName = paste0("DMpbRegGE_model_c",cluster))
    ...
    assign(value = data.frame(res_full), x = paste0("res_full_aggRec_DMpbRegGE_c",cluster))
    plotModelHistsNoFDR(get(paste0("res_full_aggRec_DMpbRegGE_c",cluster)), 
                        folderName = paste0(figDir,"c",cluster,"/"), 
                        modelName = paste0("DMpbRegGE_model_AgRec_c",cluster))
    ...
    assign(value = data.frame(res_full), x = paste0("res_full_groom_DMpbRegGE_c",cluster))
    plotModelHistsNoFDR(get(paste0("res_full_groom_DMpbRegGE_c",cluster)), 
                        folderName = paste0(figDir,"c",cluster,"/"), 
                        modelName = paste0("DMpbRegGE_model_groom_c",cluster))

    assign(value = metadata, x = paste0("metadata_DMpbRegGE_c",cluster))
    ...
    for (model in c("elo","agRec","groom")) {
      ...
      if ( model == "elo") {
        emmremlResults <- get(paste0("res_full_DMpbRegGE_c",cluster))
        ...
        assign(value = emmremlResults, x = paste0("res_full_DMpbRegGE_c",cluster))
  
      } else if ( model == "agRec" ) {
        emmremlResults <- get(paste0("res_full_aggRec_DMpbRegGE_c",cluster))
        ...
        assign(value = emmremlResults, x = paste0("res_full_aggRec_DMpbRegGE_c",cluster))
        
      } else {
        emmremlResults <- get(paste0("res_full_groom_DMpbRegGE_c",cluster))
        ...
        assign(value = emmremlResults, x = paste0("res_full_groom_DMpbRegGE_c",cluster))
...
```{r doublecheck 02 NK cluster agRec}

#res_full_aggRec_DMpbRegGE_c02_FIRSTRUN <- res_full_aggRec_DMpbRegGE_c02


hist(res_full_aggRec_DMpbRegGE_c02_FIRSTRUN$p_value_trtLPS.aggRec_cent, breaks = 100)
hist(res_full_aggRec_DMpbRegGE_c02_FIRSTRUN$p_value_trtNC.aggRec_cent, breaks = 100)

hist(qvalue(res_full_aggRec_DMpbRegGE_c02_FIRSTRUN$p_value_trtLPS.aggRec_cent)$qvalues, breaks = 100)
hist(qvalue(res_full_aggRec_DMpbRegGE_c02_FIRSTRUN$p_value_trtNC.aggRec_cent)$qvalues, breaks = 100)


hist(res_full_aggRec_DMpbRegGE_c02$p_value_trtLPS.aggRec_cent, breaks = 100)
hist(res_full_aggRec_DMpbRegGE_c02$p_value_trtNC.aggRec_cent, breaks = 100)

hist(qvalue(res_full_aggRec_DMpbRegGE_c02$p_value_trtLPS.aggRec_cent)$qvalues, breaks = 100)
hist(qvalue(res_full_aggRec_DMpbRegGE_c02$p_value_trtNC.aggRec_cent)$qvalues, breaks = 100)

hist(res_full_aggRec_DMpbRegGE_c02$fdr_agRecNestNC, breaks = 100)
hist(res_full_aggRec_DMpbRegGE_c02$fdr_agRecNestLPS, breaks = 100)
...
plotModelHists <- function(emmremlDF, folderName = "~/Downloads/", modelName = "model", 
                           behaveMetric = "behavior", cluster = "99", iters = 100) {
...
for (b in c("elo","agRec","groom")) {
  for (c in c("00","14","02","05","06","07")) {
    ...
    if (b == "elo") {
      emmremlOutput <- get(paste0("res_full_DMpbRegGE_c",c))
    } else if (b == "agRec") {
      emmremlOutput <- get(paste0("res_full_aggRec_DMpbRegGE_c",c))
    } else {
      emmremlOutput <- get(paste0("res_full_groom_DMpbRegGE_c",c))
    }
    
    assign(x = paste0("shuffled_",b,"_trt_pval_c",c), value = read.table(file = paste0(workDir,"permutations/shuffled_",b,"_trt_pval_c",c,".txt"), header = T))
    assign(x = paste0("shuffled_",b,"_LPS_pval_c",c), value = read.table(file = paste0(workDir,"permutations/shuffled_",b,"_LPS_pval_c",c,".txt"), header = T))
    assign(x = paste0("shuffled_",b,"_NC_pval_c",c), value = read.table(file = paste0(workDir,"permutations/shuffled_",b,"_NC_pval_c",c,".txt"), header = T))
...
for (c in 1:dim(clusterCellID)[1]) {
  ...
  for (f in FDRs) {
    DMPBemmreml <- get(paste0("res_full_DMpbRegGE_c",cluster))
    ...
    DMPBemmreml <- get(paste0("res_full_aggRec_DMpbRegGE_c",cluster))
    ...
    DMPBemmreml <- get(paste0("res_full_groom_DMpbRegGE_c",cluster))
...
write.table(eloncFDR, file = paste0(workDir,"eloncFDRcounts.txt"), quote = F, sep = ",")
write.table(agRecncFDR, file = paste0(workDir,"agRecncFDRcounts.txt"), quote = F, sep = ",")
write.table(groomncFDR, file = paste0(workDir,"groomncFDRcounts.txt"), quote = F, sep = ",")
...
for (variable in c("aggRec","elo","groom")) {
  ...
  for (cluster in c("00","14","02","05","06","07")) {
    ...
    if (variable == "elo") {
      emrml <- get(paste0("res_full_DMpbRegGE_c",cluster))
    } else {
    ...
    emrml$NCsig <- emrml[,20] < 0.20
    emrml$LPSsig <- emrml[,21] < 0.20
    ...
    for (treatment in c("NC","LPS")) {
      ...
      ggsave(paste0(figDir,"/sigBetas_hist_fdr20_",variable,"x",treatment,"_c",cluster,".png"))
    }
...
for (cluster in c("00","02","14","05","06","07")) {
  ...
  for (variable in c("aggRec","groom","elo")) {
    ...
    ncFET <- matrix(c(sum(ncResults$combSig == "both"),sum(ncResults$combSig == "mean"),sum(ncResults$combSig == "dm"),sum(ncResults$combSig == "neither")), ncol = 2, byrow = T)
    ncFETresults <- fisher.test(ncFET)
    lpsFET <- matrix(c(sum(lpsResults$combSig == "both"),sum(lpsResults$combSig == "mean"),sum(lpsResults$combSig == "dm"),sum(lpsResults$combSig == "neither")), ncol = 2, byrow = T)
    lpsFETresults <- fisher.test(lpsFET)
    ...
    if (variable == "elo") {
      ...
      ncFET <- matrix(c(sum(ncResults$combSig == "both"),
                        sum(ncResults$combSig == "mean"),
                        sum(ncResults$combSig == "dm"),
                        sum(ncResults$combSig == "neither")), ncol = 2, byrow = T)
      ncFETresults <- fisher.test(ncFET)
            
      
      cellClustFET <- c(cellClustFET, cluster)
      behaveVarFET <- c(behaveVarFET, variable)
      treatCondFET <- c(treatCondFET, "NC")
      dmSigFET <- c(dmSigFET,sum(ncResults$combSig == "dm"))
      mnSigFET <- c(mnSigFET,sum(ncResults$combSig == "mean"))
      bothSigFET <- c(bothSigFET,sum(ncResults$combSig == "both"))
      neitherSigFET <- c(neitherSigFET,sum(ncResults$combSig == "neither"))
      pvalFET <- c(pvalFET,ncFETresults$p.value)
      oddsFET <- c(oddsFET,ncFETresults$estimate)
      confLoFET <- c(confLoFET,lpsFETresults$conf.int[1])
      confHiFET <- c(confHiFET,lpsFETresults$conf.int[2])


      
      
      lpsFET <- matrix(c(sum(lpsResults$combSig == "both"),
                         sum(lpsResults$combSig == "mean"),
                         sum(lpsResults$combSig == "dm"),
                         sum(lpsResults$combSig == "neither")), ncol = 2, byrow = T)
      lpsFETresults <- fisher.test(lpsFET)

      
      cellClustFET <- c(cellClustFET, cluster)
      behaveVarFET <- c(behaveVarFET, variable)
      treatCondFET <- c(treatCondFET, "LPS")
      dmSigFET <- c(dmSigFET,sum(lpsResults$combSig == "dm"))
      mnSigFET <- c(mnSigFET,sum(lpsResults$combSig == "mean"))
      bothSigFET <- c(bothSigFET,sum(lpsResults$combSig == "both"))
      neitherSigFET <- c(neitherSigFET,sum(lpsResults$combSig == "neither"))
      pvalFET <- c(pvalFET,lpsFETresults$p.value)
      oddsFET <- c(oddsFET,lpsFETresults$estimate)
      confLoFET <- c(confLoFET,lpsFETresults$conf.int[1])
      confHiFET <- c(confHiFET,lpsFETresults$conf.int[2])
    }
  }
  ...
  sigGeneFETdf <- as.data.frame(cellClustFET)
  
  sigGeneFETdf$behaveVarFET <- behaveVarFET
  sigGeneFETdf$treatCondFET <- treatCondFET
  sigGeneFETdf$dmSigFET <- dmSigFET
  sigGeneFETdf$mnSigFET <- mnSigFET
  sigGeneFETdf$bothSigFET <- bothSigFET
  sigGeneFETdf$neitherSigFET <- neitherSigFET
  sigGeneFETdf$oddsFET <- oddsFET
  sigGeneFETdf$confLoFET <- confLoFET
  sigGeneFETdf$confHiFET <- confHiFET
  sigGeneFETdf$pvalFET <- pvalFET
  
  write.table(x = sigGeneFETdf, file = paste0(workDir,"sigGenesFETDataFrame.txt"))
...
for (cluster in c("00","14","02","05","06","07")) {
  ...
  #take residuals of DM~mean relationship  
  sge_LPS_DMpbResid_matrix <- sge_LPS_DMpb_matrix
  sge_NC_DMpbResid_matrix <- sge_NC_DMpb_matrix

    
  for (g in 1:dim(sge_LPS_DMpb_matrix)) {
    tmpData <- as.data.frame(cbind(t(sge_NC_pbMean_matrix[g,]),t(sge_NC_DMpb_matrix[g,])))
    colnames(tmpData) <- c("mean","DM")
    sge_NC_DMpbResid_matrix[g,] <- lm(tmpData$DM ~ tmpData$mean)$residuals
  ...
  for (i in 1:length(indIDs)) {
    ...
    rLPS <- cor(sge_LPS_DMpbResid_matrix[,i],sge_LPS_pbMean_matrix[,i])
    rNC <- cor(sge_NC_DMpbResid_matrix[,i],sge_NC_pbMean_matrix[,i])
    rCmb <- cor(c(sge_LPS_DMpbResid_matrix[,i],sge_NC_DMpbResid_matrix[,i]),
                c(sge_LPS_pbMean_matrix[,i],sge_NC_pbMean_matrix[,i]))
...
dir.create(paste0(workDir,"figures/gsea"))
...
for (cluster in clusterCellID$cluster) {
  ...
  dir.create(paste0(workDir,"figures/gsea/c",cluster))
    ...
    if( lmVar == "trt") {
      sampleBetas$Betas <- DMPBemmreml[,2] / sqrt(DMPBemmreml[,8])
    } else {
      sampleBetas$Betas <- DMPBemmreml[,3] / sqrt(DMPBemmreml[,9])
    }
...
for (cluster in clusterCellID$cluster) {
  ...
  for (bMetric in c("aggRec","elo","groom")) {
    ...
    for (treatment in c("LPS","NC")) {
      ...
      #starting with c00, treatment | res_full_aggRec_DMpbRegGE_c00
      if (bMetric == "elo") {
        DMPBemmreml <- get(paste0("res_full_DMpbRegGE_c",cluster))
      } else {
        DMPBemmreml <- get(paste0("res_full_",bMetric,"_DMpbRegGE_c",cluster))
      }
      ...
      if ( lmVar == "trt") {
        ...
        if ( treatment == "LPS") {
          sampleBetas$Betas <- DMPBemmreml[,4] / sqrt(DMPBemmreml[,10])
        } else {
          sampleBetas$Betas <- DMPBemmreml[,3] / sqrt(DMPBemmreml[,9])
        }
      }
...
for (cluster in c("00","14","02","05","06","07")) {
  ...
  ##run this for agRec and for groom
    ...
    assign(x = paste0("res_full_",variable,"_DMpbRegGE_c",cluster),
           value = read.table(paste0(workDir,"cluster/results/results_c",cluster,"_",variable,"_emmreml")))
    
    trtEmrml <- get(paste0("res_full_DMpbRegGE_c",cluster))
    behaveEmrml <- get(paste0("res_full_",variable,"_DMpbRegGE_c",cluster))
...
for (cluster in clusterCellID$cluster) {
  ...
  agRecEmmreml <- get(paste0("res_full_agRec_DMpbRegGE_c",cluster))
  groomEmmreml <- get(paste0("res_full_groom_DMpbRegGE_c",cluster))
...
```{r plots for SGE}
#comparing DM betas and mean betas
cluster <- "14"
variable <- "elo"
...
dmpbDataNC <- as.data.frame(emrml[,22])
colnames(dmpbDataNC) <- "TRTstdBetaDM"
dmpbDataNC$gene <- rownames(emrml)
    
expDataNC <- as.data.frame(paulResults[,32])
colnames(expDataNC) <- "TRTstdBetaMean"
expDataNC$gene <- rownames(paulResults)
    
allResults <- merge(expDataNC, dmpbDataNC, by = "gene")



ggplot(data = allResults, aes(x = TRTstdBetaMean, y = TRTstdBetaDM)) +
  geom_point(alpha = .5) +
  geom_smooth(method = "lm") +
  xlab(paste0("Mean std Treatment Betas")) + 
  ylab(paste0("DM std Treatment Betas")) +
  scale_color_manual(values = c("black",gg3col[3:1])) +
  geom_text(aes(x = .9*min(TRTstdBetaMean), y = .9*max(TRTstdBetaDM), 
                label = paste0("R=",round(cor(TRTstdBetaMean,TRTstdBetaDM),digits = 3),
                               ", p=",signif(cor.test(TRTstdBetaMean,TRTstdBetaDM)$p.value,digits = 2))),
            hjust = 0) +
  theme(legend.position = "none") +
  ggtitle(paste0("c",cluster," - Treatment"))

ggsave(paste0(figDir,"/stdBetas_DM_v_mean_Treatment_c",cluster,".png"))

focalGene <- "PIM2"

ggplot(data = allResults, aes(x = TRTstdBetaMean, y = TRTstdBetaDM)) +
  geom_point(alpha = .5) +
  geom_point(data = subset(allResults, gene == focalGene), aes(x = TRTstdBetaMean, y = TRTstdBetaDM), col = "red", size = 5, alpha = 3/4) +
  geom_smooth(method = "lm") +
  xlab(paste0("Mean std Treatment Betas")) + 
  ylab(paste0("DM std Treatment Betas")) +
  scale_color_manual(values = c("black",gg3col[3:1])) +
...
ggsave(paste0(figDir,"/stdBetas_DM_v_mean_Treatment_c",cluster,"_",focalGene,".png"))

#compare DM-mean treatment betas for significant effects (FET)



```

SGE follow up (from April 1 meeting)
1) what are the mean betas of the genes that are significantly correlated with high var - high stress?
...
for (variable in c("aggRec","groom")) {
  ...
  for (cluster in c("00","14","02","05","06","07")) {
    ...
    paulResults <- paulResults[rownames(paulResults) %in% rownames(emrml),]
    ...
    NCposRatioMeans <- mean(paulResults$NCstdBetaMean > 0)
    LPSposRatioMeans <- mean(paulResults$LPSstdBetaMean > 0)
    
    #get the same genes from the mean data
    ncGenes <- rownames(get(paste0("plotEmrml",cluster,variable,"NC")))
    lpsGenes <- rownames(get(paste0("plotEmrml",cluster,variable,"LPS")))
    
    meanNC <- paulResults[rownames(paulResults) %in% ncGenes,]
    meanLPS <- paulResults[rownames(paulResults) %in% lpsGenes,]
    ...
    for (treatment in c("NC","LPS")) {
      ...
      if (treatment == "NC") {
        ...
        meanColNum <- 32
        betaPosRatio <- NCposRatio
        betaPosRatioMean <- NCposRatioMeans
      } else {
        colNum <- 23
        meanColNum <- 33
        betaPosRatio <- LPSposRatio
        betaPosRatioMean <- LPSposRatioMeans
      }
      
      posBetas <- sum(get(paste0("plotEmrml",cluster,variable,treatment))[,colNum] > 0)
      negBetas <- sum(get(paste0("plotEmrml",cluster,variable,treatment))[,colNum] < 0)
      
      posMeanBetas <- sum(get(paste0("mean",treatment))[,meanColNum] > 0)
      negMeanBetas <- sum(get(paste0("mean",treatment))[,meanColNum] < 0)
      ...
      if (sum(posBetas,negBetas) > 0) {
        ...
        ### for Means
        #binomial test of 50-50 pos/neg betas
        posBinomNullMean <- binom.test(posMeanBetas, sum(posMeanBetas,negMeanBetas), p = .5)
        
        #binomial test of pos/neg betas from exact distribution
        posBinomBkgMean <- binom.test(posMeanBetas, sum(posMeanBetas,negMeanBetas), p = betaPosRatioMean)
        
        binomPvalMean <- posBinomBkgMean$p.value
      ...
      ggplot() +
        geom_histogram(aes(x = get(paste0("mean",treatment))[,meanColNum]), bins = 50) +
        ggtitle(paste0("Mean Betas ",treatment,":",variable," - c",cluster," - ",
                       dim(get(paste0("plotEmrml",cluster,variable,treatment)))[1]," sig genes")) +
        annotate(geom = "text", x = Inf, y = -Inf, label = posMeanBetas, size = 8, hjust = "inward", vjust = "inward") +
        annotate(geom = "text", x = -Inf, y = -Inf, label = negMeanBetas, size = 8, hjust = "inward", vjust = "inward") +
        annotate(geom = "text", x = 5, y = 75, label = paste0("p=",signif(binomPvalMean, digits = 2)), 
                 size = 8, hjust = .25, vjust = .5) +
        xlab(paste0(treatment,":",variable," - c",cluster," - std Betas")) +
        xlim(c(-8,8)) + ylim(c(0,100)) + theme(plot.title = element_text(size=24))
      
      ggsave(paste0(figDir,"/sigMeanBetas_hist_",variable,"x",treatment,"_c",cluster,".png"))
...
2) in the permutation runs, do the DM betas show a high var - high stress directionality?
a) the same genes that are sig in the regular data
b) the genes that are sig in that permutation (matched number of genes, listed by pvalue)
```{r followup 2}
clusterList <- c("00","14","02","05","06","07")
...
permCluster <- vector()
permNsig <- vector()
permSigBetaPos <- vector()
permSigBetaTot <- vector()
permBetaPos <- vector()
permBetaTot <- vector()
...
### didn't save the beta values from the permuted runs... so i need to re-permute them here
...
for (cluster in clusterList) {
  ...
  #need to run the loop locally
    ...
    #go through each of the 3 models (elo, agRec, groom) and generate a null
    for (model in c("groom","agRec")) {
      #model <- "agRec"
      modelName <- str_replace(model,"ag","agg")
      
        emrmlRes <- get(paste0("res_full_",modelName,"_DMpbRegGE_c",cluster))

        LPSsigGenes <- rownames(emrmlRes)[emrmlRes[,21] < .1]
        NCsigGenes <- rownames(emrmlRes)[emrmlRes[,20] < .1]
      ...
      for(iter in 1:iters) {
        ...
        #we register betas of the behavior at NC and at LPS alone.
        if( iter == 1 ) {
          shuffled_behave_betas_NC <- data.frame(x=res_null[,3])
          shuffled_behave_betas_LPS <- data.frame(x=res_null[,4])
          rownames(shuffled_behave_betas_NC) <- rownames(res_null)
          rownames(shuffled_behave_betas_LPS) <- rownames(res_null)
        } else {
          shuffled_behave_betas_NC <- cbind(shuffled_behave_betas_NC,x=res_null[,3])
          shuffled_behave_betas_LPS <- cbind(shuffled_behave_betas_LPS,x=res_null[,4])
        }
      }
      ...
      print(model)
      print("LPS")
      print("sig genes, permutated beta > 0")
      print(sum(unlist(shuffled_behave_betas_LPS[rownames(shuffled_behave_betas_LPS) %in% LPSsigGenes,]) > 0))
      print("sig genes, total permutated betas")
      print(length(unlist(shuffled_behave_betas_LPS[rownames(shuffled_behave_betas_LPS) %in% LPSsigGenes,])))
      
      print("all genes, permutated beta > 0")
      print(sum(unlist(shuffled_behave_betas_LPS) > 0))
      print("all genes, total permutated betas")
      print(length(unlist(shuffled_behave_betas_LPS)))
      
      permModel <- c(permModel,model)
      permTrt <- c(permTrt,"LPS")
      permCluster <- c(permCluster,cluster)
      permNsig <- c(permNsig,length(LPSsigGenes))
      permSigBetaPos <- c(permSigBetaPos,sum(unlist(shuffled_behave_betas_LPS[rownames(shuffled_behave_betas_LPS) %in% LPSsigGenes,]) > 0))
      permSigBetaTot <- c(permSigBetaTot,length(unlist(shuffled_behave_betas_LPS[rownames(shuffled_behave_betas_LPS) %in% LPSsigGenes,])))
      permBetaPos <- c(permBetaPos,sum(unlist(shuffled_behave_betas_LPS) > 0))
      permBetaTot <- c(permBetaTot,length(unlist(shuffled_behave_betas_LPS)))

      
      print(model)      
      print("NC")
      print("sig genes, permutated beta > 0")
      print(sum(unlist(shuffled_behave_betas_NC[rownames(shuffled_behave_betas_NC) %in% NCsigGenes,]) > 0))
      print("sig genes, total permutated betas")
      print(length(unlist(shuffled_behave_betas_NC[rownames(shuffled_behave_betas_NC) %in% NCsigGenes,])))

      print("all genes, permutated beta > 0")
      print(sum(unlist(shuffled_behave_betas_NC) > 0))
      print("all genes, total permutated betas")
      print(length(unlist(shuffled_behave_betas_NC)))
      
      
      permModel <- c(permModel,model)
      permTrt <- c(permTrt,"NC")
      permCluster <- c(permCluster,cluster)
      permNsig <- c(permNsig,length(NCsigGenes))
      permSigBetaPos <- c(permSigBetaPos,sum(unlist(shuffled_behave_betas_NC[rownames(shuffled_behave_betas_NC) %in% NCsigGenes,]) > 0))
      permSigBetaTot <- c(permSigBetaTot,length(unlist(shuffled_behave_betas_NC[rownames(shuffled_behave_betas_NC) %in% NCsigGenes,])))
      permBetaPos <- c(permBetaPos,sum(unlist(shuffled_behave_betas_NC) > 0))
      permBetaTot <- c(permBetaTot,length(unlist(shuffled_behave_betas_NC)))
...
permutationBetas <- as.data.frame(cbind(permModel,permTrt,permCluster,permNsig,permSigBetaPos,permSigBetaTot,permBetaPos,permBetaTot))

write.table(permutationBetas, paste0(workDir,"permutationBetas.txt"))

permutationBetas$permNsig <- as.numeric(permutationBetas$permNsig)
permutationBetas$permSigBetaPos <- as.numeric(permutationBetas$permSigBetaPos)
permutationBetas$permSigBetaTot <- as.numeric(permutationBetas$permSigBetaTot)
permutationBetas$permBetaPos <- as.numeric(permutationBetas$permBetaPos)
permutationBetas$permBetaTot <- as.numeric(permutationBetas$permBetaTot)

permutationBetas$permSigBetaProp <- permutationBetas$permSigBetaPos / permutationBetas$permSigBetaTot
permutationBetas$permBetaProp <- permutationBetas$permBetaPos / permutationBetas$permBetaTot
permutationBetas$flatBinomP <- NA
permutationBetas$adjBinomP <- NA

for (n in 1:dim(permutationBetas)[1]) {
  
  if (permutationBetas$permSigBetaTot[n] > 0) {
    #binomial test of permutation data
    bTest <- binom.test(x = permutationBetas$permSigBetaPos[n], n = permutationBetas$permSigBetaTot[n], 
               p = permutationBetas$permBetaPos[n] / permutationBetas$permBetaTot[n])
    permutationBetas$adjBinomP[n] <- bTest$p.value
    
    bTest <- binom.test(x = permutationBetas$permSigBetaPos[n], n = permutationBetas$permSigBetaTot[n], 
               p = .5)
    permutationBetas$flatBinomP[n] <- bTest$p.value
    
    
  }
}

write.table(permutationBetas, paste0(workDir,"permutationBetas_wBinom.txt"))
...
for (b in c("elo","groom","agRec")) {
  ...
  NCfdr <- read.table(paste0(workDir,b,"ncFDRCounts.txt"), sep = ",")
  LPSfdr <- read.table(paste0(workDir,b,"TrtFDRCounts.txt"), sep = ",")
...
allfdr <- read.csv(paste0(workDir,"treatmentFDRcounts.txt"))
...
```{r slide 7 beta assymetry cluster names}

betaHistNames <- betaHistDF

betaHistNames$cellClusterName <- ""

betaHistNames$cellClusterName <- ifelse(betaHistNames$cellClustChar == "0", "CD8",
                                   ifelse(betaHistNames$cellClustChar == "14", "CD4",
                                          ifelse(betaHistNames$cellClustChar == "2", "NK",
                                                 ifelse(betaHistNames$cellClustChar == "5", "Bcells", "CD14hi"))))


betaHistAgRec <- subset(betaHistNames, behaveVar == "aggRec" & cellClusterName != "Bcells")

ggplot(betaHistAgRec, aes(x = betaVal, y = cellClusterName, group = cellClusterName, color = cellClusterName, fill = cellClusterName)) +
  geom_density_ridges(stat = "binline") + scale_fill_viridis(discrete=TRUE) + theme_minimal() + scale_color_viridis(discrete=TRUE) +
  ylab("") + xlab("Significant Beta Values, Cell-to-Cell Variance") + theme(legend.position = "none") +
  ggtitle("Agonisms Received")

ggsave(paste0(figDir,"/7_sigBetas_allCells_agRec_density_names.png"))
...
ggsave(paste0(figDir,"/7_sigBetas_allCells_agRec_hist_names.png"))


betaHistGroom <- subset(betaHistNames, behaveVar == "groom" & cellClusterName != "Bcells")

ggplot(betaHistGroom, aes(x = betaVal, y = cellClusterName, group = cellClusterName, color = cellClusterName, fill = cellClusterName)) +
  geom_density_ridges(stat = "binline") + scale_fill_viridis(discrete=TRUE) + theme_minimal() + scale_color_viridis(discrete=TRUE) +
  ylab("") + xlab("Significant Beta Values, Cell-to-Cell Variance") + theme(legend.position = "none") +
  ggtitle("Overall Grooming")

ggsave(paste0(figDir,"/7_sigBetas_allCells_groom_density_names.png"))
...
ggsave(paste0(figDir,"/7_sigBetas_allCells_groom_hist_names.png"))
