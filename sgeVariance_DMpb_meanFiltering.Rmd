...
## drop bottom 25% of genes by GE pseudobulk

```{r loop through clusters to model DMpb}
figDir <- "~/Dropbox (Personal)/scRNA/SGE_DM/q25dir/figures/"
workDir <- "~/Dropbox (Personal)/scRNA/SGE_DM/q25dir/"
...
for (cluster in c("00","14","02","05","06","07")) {
  ...
  metadata$trt <- factor(metadata$trt, levels = c("NC","LPS"))
    ...
    dir.create(paste0(workDir,"figures/c",cluster))
  ...
  if ( sum(clustResults %in% sgeOutput) == 3 ) {
    ...
    for (model in c("elo","agRec","groom")) {
      ...
      write.table(shuffled_elos_pvals_LPS, file = paste0(workDir,"permutations/shuffled_",model,"_NC_pval_c",
       ...
       if ( model == "elo") {
        emmremlResultsNoPerm <- get(paste0("res_full_DMpb_c",cluster))
       } else if ( model == "agRec" ) {
        emmremlResultsNoPerm <- get(paste0("res_full_aggRec_DMpb_c",cluster))
       } else {
        emmremlResultsNoPerm <- get(paste0("res_full_groom_DMpb_c",cluster))
       }
    
      write.table(emmremlResultsNoPerm, file=paste0(workDir,paste0("results/results_c",cluster,"_",model,"_emmreml_noPerm")),row.names=T,col.names=T,quote=F,sep='\t')
