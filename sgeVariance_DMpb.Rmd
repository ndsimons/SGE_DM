...
# Behavioral Interaction with LPS -- difference in beta method
```{r}
...
#run emmreml models again, but this time across NC and LPS, subset first on genes that do show a significant treatment effect (maybe, 0.20 qvalue)

#for those genes, is there a difference in behavior~variance in or outside of LPS

pvalCutoff <- 0.05

#only run cluster 5 as a test
for (cluster in c("00","14","02","05","06","07")) {
  #cluster <- "05"
  ...
  for (variable in c("agRec","groom")){
    ...
    ### only want to use genes that were significant in the treatment condition ~0.20 FDR
    # get the list of genes from that emreml result
    sigGenes <- rownames(trtEmrml)[trtEmrml$p_value_trtLPS < pvalCutoff]
    ...
    #   tmpObj <- get(fObj)
    #   ## subset mean and variance data frames to include only the genes present in the DM data frame ##
    #   tmpObj <- subset(tmpObj, rownames(tmpObj) %in% sigGenes)
    #   
    #   ## fix MO - 77_06 ID
    #   if (sum(colnames(tmpObj) == "X77_06") > 0) {
    #     colnames(tmpObj)[which(colnames(tmpObj) == "X77_06")] <- "Mo"
    ...
    varEmrml <- get(paste0("res_full_",variable,"_DMpb_c",cluster))
    
    #varEmrml$diffBeta <- varEmrml[,4] / sqrt(varEmrml[,10]) - varEmrml[,3] / sqrt(varEmrml[,9])
    varEmrml$diffBeta <- varEmrml[,4]  - varEmrml[,3]
    
    varEmrmlSig <- varEmrml[rownames(varEmrml) %in% sigGenes,]
    
    ggplot() +
      geom_histogram(aes(x = varEmrml$diffBeta)) +
      geom_histogram(aes(x = varEmrmlSig$diffBeta), fill = "red")
    
    #which genes have betas beyond 3 std. dev
    upperLim <- mean(varEmrmlSig$diffBeta) + 3*sd(varEmrmlSig$diffBeta)
    lowerLim <- mean(varEmrmlSig$diffBeta) - 3*sd(varEmrmlSig$diffBeta)
    
    diffBetaGenes <- subset(varEmrmlSig, diffBeta < lowerLim | diffBeta > upperLim)
    
    #if there are any genes
    if ( dim(diffBetaGenes)[1] > 0 ) {
    #for all the diff genes

      for (g in 1:dim(diffBetaGenes)[1]) {
        
        geneName <- rownames(diffBetaGenes)[g]
        
        NCvars <- sge_NC_DMpb_matrix[rownames(sge_NC_DMpb_matrix) == rownames(diffBetaGenes)[g],]
        LPSvars <- sge_LPS_DMpb_matrix[rownames(sge_LPS_DMpb_matrix) == rownames(diffBetaGenes)[g],]
        
        ggplot() +
          geom_histogram(aes(x = unlist(NCvars)), fill = "blue", alpha = 1/3) +
          geom_histogram(aes(x = unlist(LPSvars)), fill = "red", alpha = 1/3)
    
        #only need metadata for these individuals
        sigMetadata <- metadata[metadata$ID %in% names(NCvars),]
        
        if (variable == "agRec") {
          sigMetadata <- sigMetadata[,c(1:4,8)]
        } else {
          sigMetadata <- sigMetadata[,c(1:4,9)]
        }
        
        sigMetadata$LPSvar <- 0
        sigMetadata$NCvar <- 0
        
        for (i in 1:dim(sigMetadata)[1]) {
          varID <- which(names(NCvars) == sigMetadata$ID[i])
          sigMetadata$LPSvar[i] <- as.numeric(LPSvars[varID])
          sigMetadata$NCvar[i] <- as.numeric(NCvars[varID])
        }
        
        ggplot() +
          geom_point(aes(x = sigMetadata[,5], y = sigMetadata$NCvar), col = "blue") +
          geom_point(aes(x = sigMetadata[,5], y = sigMetadata$LPSvar), col = "red") +
          geom_smooth(aes(x = sigMetadata[,5], y = sigMetadata$NCvar), method = "lm", col = "blue") +
          geom_smooth(aes(x = sigMetadata[,5], y = sigMetadata$LPSvar), method = "lm", col = "red") +
          xlab(paste0(variable," (Centered)")) + ylab("DMpb (Red = LPS, Blue = NC)") +
          ggtitle(paste0("NC-LPS interaction in ",geneName," metric:",variable))
        
        ggsave(paste0(workDir,"figures/responseEffects/interaction_xy_c",cluster,"_",variable,"_",geneName,".png"), 
             width = 8, height = 6)
      }
    }
...
```
## Run an Emreml model where "y" is just the difference in variance, not the raw variance
```{r emmreml for diff in var}
#number of permutations
iters <- 100
pvalCutoff <- 0.01
...
for (cluster in c("00","14","02","05","06","07")) {
  ...
  #calculate the DMdiff matrix | LPS - NC
  sge_DMdiff_matrix <- sge_LPS_DMpb_matrix - sge_NC_DMpb_matrix
  ...
  ## regress out group effects from DMpb ##
    design <- model.matrix(~metadata$group)
    fit <-lmFit(sge_DMdiff_matrix,design)
    intercepts=data.frame(eBayes(fit))[,1]
    resid_DMdiff=apply(residuals.MArrayLM(object=fit, sge_DMdiff_matrix),2,function(x){x+intercepts})
    ...
    #Agonism Received
    design = model.matrix(~aggRec_cent + age_centered, data=metadata)
    
    #Declare object res_full to store in it the model results: beta coefficients, standard deviations and p values
    res_full=resid_DMdiff[,1:(3*ncol(design))]
    colnames(res_full)[1:ncol(design)]=paste0("beta_",colnames(design))
    colnames(res_full)[(ncol(design)+1):(2*ncol(design))]=paste0("sdev_",colnames(design))
    colnames(res_full)[((2*ncol(design))+1):(3*ncol(design))]=paste0("p_value_",colnames(design))
    
    #Declare object random_effects to store in it the individual-wise u random effects
    random_effects=resid_DMdiff[,1:length(indIDs)]
    colnames(random_effects)=metadata[1:length(indIDs),'ID']
    ...
    for(i in 1:nrow(resid_DMdiff))
    {
      emma=emmreml(y=resid_DMdiff[i,],X=design,Z=as.matrix(Z),K=as.matrix(K),varbetahat=T,varuhat=T,PEVuhat=T,test=T)
      random_effects[i,]=t(emma$uhat)
      res_full[i,]=t(c(emma$betahat,emma$varbetahat,emma$pvalbeta[,"none"]))
    }
    
    assign(value = data.frame(res_full), x = paste0("res_full_aggRec_DMdiff_c",cluster))
    #plotModelHists(get(paste0("res_full_aggRec_DMdiff_c",cluster)), folderName = paste0(figDir,"c",cluster,"/"), modelName = paste0("DMdiff_model_AgRec_c",cluster))
    
    ggplot() +
     geom_histogram(aes(x = res_full[,8]), bins = 100) + xlab("pvalues") +
          ggtitle(paste0("LPS - NC model ",cluster,", agRec"), subtitle = paste0("genes below ",pvalCutoff,", n = ",dim(res_full)[1]))
    ggsave(paste0(workDir,"figures/responseEffects/pval_hist_",cluster,"_agRec.png"), 
             width = 6, height = 4)
    
    #Overall Grooming
    
    design = model.matrix(~groom_cent + age_centered, data=metadata)
    ...
    assign(value = data.frame(res_full), x = paste0("res_full_groom_DMdiff_c",cluster))
    #plotModelHists(get(paste0("res_full_groom_DMdiff_c",cluster)), folderName = paste0(figDir,"c",cluster,"/"), modelName = paste0("DMdiff_model_groom_c",cluster))
    
    ggplot() +
     geom_histogram(aes(x = res_full[,8]), bins = 100) + xlab("pvalues") +
          ggtitle(paste0("LPS - NC model ",cluster,", groom"), subtitle = paste0("genes below ",pvalCutoff,", n = ",dim(res_full)[1]))
    ggsave(paste0(workDir,"figures/responseEffects/pval_hist_",cluster,"_groom.png"), 
             width = 6, height = 4)
...
ggplot() +
  geom_histogram(aes(x = res_full_aggRec_DMdiff_c14$p_value_aggRec_cent), bins = 100)

ggplot() +
  geom_histogram(aes(x = res_full_groom_DMdiff_c14$p_value_groom_cent), bins = 100)

ggplot() +
  geom_histogram(aes(x = res_full_aggRec_DMdiff_c14$p_value_age_centered), bins = 100)

ggplot() +
  geom_histogram(aes(x = res_full_groom_DMdiff_c14$p_value_age_centered), bins = 100)
