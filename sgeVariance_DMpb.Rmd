...
##### USE THIS SAME LOOP TO GET THE CORRELATION INFO FOR THE PLOTS... JUST ADD CLUSTER, VAR, TRT etc
...
dmMeanCors <- data.frame(clusterID,animalID,NCcor,LPScor,COMBcor)

ggplot(data = dmMeanCors, aes(y = COMBcor, x = clusterID)) +
  geom_point() +
  geom_boxplot()

ggplot(data = dmMeanCors, aes(y = LPScor, x = clusterID)) +
  geom_point() +
  geom_boxplot()

ggplot(data = dmMeanCors, aes(y = NCcor, x = clusterID)) +
  geom_point() +
  geom_boxplot()
...
do the underlying data, on an individual by individual basis, reflect the same negative correlation between DMpb and pbMean?
```{r}
cellType <- vector()
DMpbMeanR <- vector()
DMpbMeanP <- vector()
