...
for (cluster in c("00","14","02","05","06","07")) {
  ...
  p1 <- ggplot() +
    geom_point(aes(x = LPS1, y = NC1, col = sigSet1), alpha = .2) +
    geom_smooth(aes(x = LPS1, y = NC1), method = "lm") +
    xlab("LPS:agRec std Betas") + ylab("NC:agRec std Betas") +
    scale_color_manual(values = c("black","blue","blue","red")) +
    theme(legend.position = "none") +
    ggtitle(paste0("c",cluster," - agRec"), subtitle = paste0("NC:",sum(NC1sig)," LPS:",sum(LPS1sig)," both:",sum(sigSet1 == "both")))
  ...
  p2 <- ggplot() +
    geom_point(aes(x = LPS2, y = NC2, col = sigSet2), alpha = .2) +
    geom_smooth(aes(x = LPS2, y = NC2), method = "lm") +
    xlab("LPS:agAsym std Betas") + ylab("NC:agAsym std Betas") +
    scale_color_manual(values = c("black","blue","blue","red")) +
    theme(legend.position = "none") +
    ggtitle(paste0("c",cluster," - agAsym"), subtitle = paste0("NC:",sum(NC2sig)," LPS:",sum(LPS2sig)," both:",sum(sigSet2 == "both")))
  ...
  p3 <- ggplot() +
    geom_point(aes(x = LPS3, y = NC3, col = sigSet3), alpha = .2) +
  ...
  p4 <- ggplot() +
    geom_point(aes(x = LPS4, y = NC4, col = sigSet4), alpha = .2) +
...
for (variable in c("aggRec","elo","groom")) {
  ...
  for (cluster in c("00","14","02","05","06","07")) {
    ...
    emrml$NCstdBeta <- emrml[,4] / sqrt(emrml[,10])
    emrml$LPSstdBeta <- emrml[,3] / sqrt(emrml[,9])
    
    emrml$NCsig <- qvalue(p = emrml[,16], fdr.level = .2)$significant
    emrml$LPSsig <- qvalue(p = emrml[,15], fdr.level = .2)$significant 
    ...
    for (treatment in c("NC","LPS")) {
      ...
      ggplot() +
        ...
        geom_text(aes(x = 6, y = 75, label = posBetas), size = 10) +
        geom_text(aes(x = -6, y = 75, label = negBetas), size = 10) +
    ...
    if (variable == "elo") {
      ...
      ggplot() +
        ...
        geom_text(aes(x = 9, y = 250, label = negBetas), size = 10) +
        geom_text(aes(x = -9, y = 250, label = posBetas), size = 10) +
...
clusterCellID$aggRecBetas <- c("~/Dropbox (Personal)/scRNA/SGE_DM/paulData/res_fdr_celltype_CD8_A_modelnum_1_covariate_gc.overall.agg.rec_nperms_100.RDS",
                               ...
                               "~/Dropbox (Personal)/scRNA/SGE_DM/paulData/res_fdr_celltype_CD8_B_modelnum_1_covariate_gc.overall.agg.rec_nperms_100.RDS",
...
clusterCellID$groomBetas <- c("~/Dropbox (Personal)/scRNA/SGE_DM/paulData/res_fdr_celltype_CD8_A_modelnum_1_covariate_gc.overall.groom_nperms_100.RDS",
                              ...
                              "~/Dropbox (Personal)/scRNA/SGE_DM/paulData/res_fdr_celltype_CD8_B_modelnum_1_covariate_gc.overall.groom_nperms_100.RDS",
...
for (cluster in c("00","02","14","05","06","07")) {
  ...
  for (variable in c("aggRec","groom","elo")) {
    ...
    ggplot(data = ncResults, aes(x = NCstdBetaMean, y = NCstdBetaDM)) +
      geom_point(aes(col = combSig), alpha = .2) +
