...
for (c in c(1:3,5:7)) {
  ...
  DMPBemmreml <- get(paste0("res_full_DMpb_c",cluster))
  eloList <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,15], fdr.level = f)$significant]
  DMPBemmreml <- get(paste0("res_full_aggAsym_DMpb_c",cluster))
  agAsymList <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,15], fdr.level = f)$significant]
  DMPBemmreml <- get(paste0("res_full_aggRec_DMpb_c",cluster))
  agRecList <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,15], fdr.level = f)$significant]
  DMPBemmreml <- get(paste0("res_full_groom_DMpb_c",cluster))
  groomList <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,15], fdr.level = f)$significant]
  ...
  MyVenn$labels <- c(paste0("groom\n",length(groomList)),
                     ...
                     paste0("elo\n",length(eloList)))
  #MyVenn$labels <- c("A\n22","B\n7","C\n5","D\n58")
  plot(MyVenn, main = paste0("c",cluster," - ",cellID))
}
