...
#starting with c00, agonisms received
sampleBetas <- as.data.frame(rownames(res_full_aggRec_DMpb_c00))
sampleBetas$Betas <- res_full_aggRec_DMpb_c00$beta_trtNC
...
for (r in 1:2) {
  if(sum(output$p_values[,r] < .05) > 0) {
    for (g in which(output$p_values[,r] < .05)) {
      ...
      ggsave(paste0(workDir,"figures/gsea/","c00","/gsea_stickPlot_c00_trtNC_",pathwayName,".png"), width = 6, height = 4)
    }
  }
}
...
cbind(as.character(GO_res$GO),gseaC00trtNC$p_values,gseaC00trtNC$NES_matrix_upreg[,1],gseaC00trtNC$NES_matrix_downreg[,1])
