...
```{r is there correlation between LPS and NC betas of behavior}

gg3col <- hue_pal()(3)

#across behavioral metrics
for (cluster in c("00","14","02","05","06","07")) {

  #cluster <- "00"
  
  agRec <- get(paste0("res_full_aggRec_DMpb_c",cluster))[,3] / sqrt(get(paste0("res_full_aggRec_DMpb_c",cluster))[,9])
  agAsym <- get(paste0("res_full_aggAsym_DMpb_c",cluster))[,3] / sqrt(get(paste0("res_full_aggAsym_DMpb_c",cluster))[,9]) 
  groom <- get(paste0("res_full_groom_DMpb_c",cluster))[,3] / sqrt(get(paste0("res_full_groom_DMpb_c",cluster))[,9])
  elo <- get(paste0("res_full_DMpb_c",cluster))[,3] / sqrt(get(paste0("res_full_DMpb_c",cluster))[,9])
  
  corBetas <- cor(cbind(-agRec,agAsym,groom,elo))
  
  colnames(corBetas) <- rownames(corBetas) <- c("-agRec","agAsym","groom","elo")
  
  ggcorrplot(corBetas) + ggtitle(paste0("c",cluster," - ",clusterCellID$cellID[(which(clusterCellID$cluster == cluster))]))
  
  ggsave(paste0(figDir,"/LPS_behavior_beta_correlations_c",cluster,".png"))

  
  agRec <- get(paste0("res_full_aggRec_DMpb_c",cluster))[,4] / sqrt(get(paste0("res_full_aggRec_DMpb_c",cluster))[,10])
  agAsym <- get(paste0("res_full_aggAsym_DMpb_c",cluster))[,4] / sqrt(get(paste0("res_full_aggAsym_DMpb_c",cluster))[,10]) 
  groom <- get(paste0("res_full_groom_DMpb_c",cluster))[,4] / sqrt(get(paste0("res_full_groom_DMpb_c",cluster))[,10])
  elo <- get(paste0("res_full_DMpb_c",cluster))[,4] / sqrt(get(paste0("res_full_DMpb_c",cluster))[,10])
  ...
  ggsave(paste0(figDir,"/NC_behavior_beta_correlations_c",cluster,".png"))
  
  
  ##NC v LPS within treatments
  
  NC1 <- get(paste0("res_full_aggRec_DMpb_c",cluster))[,4] / sqrt(get(paste0("res_full_aggRec_DMpb_c",cluster))[,10])
  NC1sig <- qvalue(p = get(paste0("res_full_aggRec_DMpb_c",cluster))[,16], fdr.level = .2)$significant
  LPS1 <- get(paste0("res_full_aggRec_DMpb_c",cluster))[,3] / sqrt(get(paste0("res_full_aggRec_DMpb_c",cluster))[,9])
  LPS1sig <- qvalue(p = get(paste0("res_full_aggRec_DMpb_c",cluster))[,15], fdr.level = .2)$significant
  sigSet1 <- ifelse(NC1sig & LPS1sig, "both", ifelse(LPS1sig, "lps", ifelse(NC1sig, "nc", "neither")))
  sigColor1 <- ifelse(NC1sig & LPS1sig, "#F8766D", ifelse(LPS1sig, "#619CFF", ifelse(NC1sig, "#00BA38", "#000000")))
  #sigSet1 <- factor(sigSet1, levels = c("neither","lps","nc","both"))
  
  p1 <- ggplot() +
    geom_point(aes(x = LPS1, y = NC1, col = sigColor1), alpha = .5) +
    geom_smooth(aes(x = LPS1, y = NC1), method = "lm") +
    annotate(geom = 'text', x = -Inf, y = Inf, hjust = "inward", vjust = "inward", size = 3,
             label = paste0("R=",round(cor.test(NC1,LPS1)$estimate, digits = 3),
                            " p=",signif(cor.test(NC1,LPS1)$p.value, digits = 2))) +
    xlab("LPS:agRec std Betas") + ylab("NC:agRec std Betas") +
    scale_color_manual(values = c("black",gg3col[3:1])) +
    theme(legend.position = "none") +
    ggtitle(paste0("c",cluster," - agRec"), 
            subtitle = paste0("NC:",sum(NC1sig)," LPS:",sum(LPS1sig)," both:",sum(sigSet1 == "both")))
  
  NC2 <- get(paste0("res_full_aggAsym_DMpb_c",cluster))[,4] / sqrt(get(paste0("res_full_aggAsym_DMpb_c",cluster))[,10])
  NC2sig <- qvalue(p = get(paste0("res_full_aggAsym_DMpb_c",cluster))[,16], fdr.level = .2)$significant
  LPS2 <- get(paste0("res_full_aggAsym_DMpb_c",cluster))[,3] / sqrt(get(paste0("res_full_aggAsym_DMpb_c",cluster))[,9])
  LPS2sig <- qvalue(p = get(paste0("res_full_aggAsym_DMpb_c",cluster))[,15], fdr.level = .2)$significant
  sigSet2 <- ifelse(NC2sig & LPS2sig, "both", ifelse(LPS2sig, "lps", ifelse(NC2sig, "nc", "neither")))
  sigColor2 <- ifelse(NC2sig & LPS2sig, "#F8766D", ifelse(LPS2sig, "#619CFF", ifelse(NC2sig, "#00BA38", "#000000")))
  sigSet2 <- factor(sigSet2, levels = c("neither","lps","nc","both"))
  
  p2 <- ggplot() +
    geom_point(aes(x = LPS2, y = NC2, col = sigColor2), alpha = .5) +
    geom_smooth(aes(x = LPS2, y = NC2), method = "lm") +
    annotate(geom = 'text', x = -Inf, y = Inf, hjust = "inward", vjust = "inward", size = 3,
             label = paste0("R=",round(cor.test(NC2,LPS2)$estimate, digits = 3),
                            " p=",signif(cor.test(NC2,LPS2)$p.value, digits = 2))) +
    xlab("LPS:agAsym std Betas") + ylab("NC:agAsym std Betas") +
    scale_color_manual(values = c("black",gg3col[3:1])) +
    theme(legend.position = "none") +
    ggtitle(paste0("c",cluster," - agAsym"), 
            subtitle = paste0("NC:",sum(NC2sig)," LPS:",sum(LPS2sig)," both:",sum(sigSet2 == "both")))
  
  NC3 <- get(paste0("res_full_groom_DMpb_c",cluster))[,4] / sqrt(get(paste0("res_full_groom_DMpb_c",cluster))[,10])
  NC3sig <- qvalue(p = get(paste0("res_full_groom_DMpb_c",cluster))[,16], fdr.level = .2)$significant
  LPS3 <- get(paste0("res_full_groom_DMpb_c",cluster))[,3] / sqrt(get(paste0("res_full_groom_DMpb_c",cluster))[,9])
  LPS3sig <- qvalue(p = get(paste0("res_full_groom_DMpb_c",cluster))[,15], fdr.level = .2)$significant
  sigSet3 <- ifelse(NC3sig & LPS3sig, "both", ifelse(LPS3sig, "lps", ifelse(NC3sig, "nc", "neither")))
  sigColor3 <- ifelse(NC3sig & LPS3sig, "#F8766D", ifelse(LPS3sig, "#619CFF", ifelse(NC3sig, "#00BA38", "#000000")))
  sigSet3 <- factor(sigSet3, levels = c("neither","lps","nc","both"))
  
  p3 <- ggplot() +
    geom_point(aes(x = LPS3, y = NC3, col = sigColor3), alpha = .5) +
    geom_smooth(aes(x = LPS3, y = NC3), method = "lm") +
    annotate(geom = 'text', x = -Inf, y = Inf, hjust = "inward", vjust = "inward", size = 3,
             label = paste0("R=",round(cor.test(NC3,LPS3)$estimate, digits = 3),
                            " p=",signif(cor.test(NC3,LPS3)$p.value, digits = 2))) +
    xlab("LPS:groom std Betas") + ylab("NC:groom std Betas") +
    scale_color_manual(values = c("black",gg3col[3:1])) +
    theme(legend.position = "none") +
    ggtitle(paste0("c",cluster," - groom"), subtitle = paste0("NC:",sum(NC3sig)," LPS:",sum(LPS3sig)," both:",sum(sigSet3 == "both")))
  
  NC4 <- get(paste0("res_full_DMpb_c",cluster))[,4] / sqrt(get(paste0("res_full_DMpb_c",cluster))[,10])
  NC4sig <- qvalue(p = get(paste0("res_full_DMpb_c",cluster))[,16], fdr.level = .2)$significant
  LPS4 <- get(paste0("res_full_DMpb_c",cluster))[,3] / sqrt(get(paste0("res_full_DMpb_c",cluster))[,9])
  LPS4sig <- qvalue(p = get(paste0("res_full_DMpb_c",cluster))[,15], fdr.level = .2)$significant
  sigSet4 <- ifelse(NC4sig & LPS4sig, "both", ifelse(LPS4sig, "lps", ifelse(NC4sig, "nc", "neither")))
  sigColor4 <- ifelse(NC4sig & LPS4sig, "#F8766D", ifelse(LPS4sig, "#619CFF", ifelse(NC4sig, "#00BA38", "#000000")))
  sigSet4 <- factor(sigSet4, levels = c("neither","lps","nc","both"))
  
  p4 <- ggplot() +
    geom_point(aes(x = LPS4, y = NC4, col = sigColor4), alpha = .5) +
    geom_smooth(aes(x = LPS4, y = NC4), method = "lm") +
    annotate(geom = 'text', x = -Inf, y = Inf, hjust = "inward", vjust = "inward", size = 3,
             label = paste0("R=",round(cor.test(NC4,LPS4)$estimate, digits = 3),
                            " p=",signif(cor.test(NC4,LPS4)$p.value, digits = 2))) +
    xlab("LPS:elo std Betas") + ylab("NC:elo std Betas") +
    scale_color_manual(values = c("black",gg3col[3:1])) +
    theme(legend.position = "none") +
    ggtitle(paste0("c",cluster," - elo"), subtitle = paste0("NC:",sum(NC4sig)," LPS:",sum(LPS4sig)," both:",sum(sigSet4 == "both")))
  
  plot_grid(p1,p2,p3,p4, ncol = 2)
  ggsave(paste0(figDir,"/LPS_v_NC_nested_behavior_betas_correlations_c",cluster,".png"))
}
...
```{r histograms with counts of significant betas}
...
for (variable in c("aggRec","elo","groom")) {
  ...
  for (cluster in c("00","14","02","05","06","07")) {
    ...
    emrml$NCsig <- qvalue(p = emrml[,15], fdr.level = .2)$significant
    emrml$LPSsig <- qvalue(p = emrml[,16], fdr.level = .2)$significant 
    ...
    if (variable == "elo") {
      ...
      emrml$TRTsig <- qvalue(p = emrml[,14], fdr.level = .2)$significant
      ...
      ggsave(paste0(figDir,"/sigBetas_hist_TRT_c",cluster,".png"))
    }
  }
}
...
#have locations of mean betas:
clusterCellID$eloBetas <- clusterCellID$cellData

betaLocation <- c("elo","aggRec","groom")
...
for (cluster in c("00","02","14","05","06","07")) {
  ...
  for (variable in c("aggRec","groom","elo")) {
    ...
    paulResults$NCsigMean <- paulResults[,28] < 0.20
    paulResults$LPSsigMean <- paulResults[,29] < 0.20
    ...
    emrml$NCstdBetaDM <- emrml[,4] / sqrt(emrml[,10])
    emrml$LPSstdBetaDM <- emrml[,3] / sqrt(emrml[,9])
    
    emrml$NCsigDM <- qvalue(p = emrml[,16], fdr.level = .2)$significant
    emrml$LPSsigDM <- qvalue(p = emrml[,15], fdr.level = .2)$significant 
    
    
    dmpbDataNC <- emrml[,c(19,21)]
    ...
    ggplot(data = ncResults, aes(x = NCstdBetaMean, y = NCstdBetaDM)) +
      ...
      ggtitle(paste0("c",cluster," - ",treatment,":",variable), 
              subtitle = paste0("mean:",sum(ncResults$NCsigMean)," DM:",sum(ncResults$NCsigDM)))
    ...
    ggplot(data = lpsResults, aes(x = LPSstdBetaMean, y = LPSstdBetaDM)) +
      ...
      theme(legend.position = "none") +
      ggtitle(paste0("c",cluster," - ",treatment,":",variable), 
              subtitle = paste0("mean:",sum(lpsResults$LPSsigMean)," DM:",sum(lpsResults$LPSsigDM)))
    ...
    if (variable == "elo") {
      for (trtFDR in c(.2,.01)) {
        
        emrml$TRTstdBetaDM <- emrml[,2] / sqrt(emrml[,8])
        emrml$TRTsigDM <- qvalue(p = emrml[,14], fdr.level = .2)$significant
        
        paulResults$TRTstdBetaMean <- paulResults[,2] / sqrt(paulResults[,10])
        paulResults$TRTsigMean <- paulResults[,31] < trtFDR
        
    
        dmpbDataTRT <- emrml[,c(23,24)]
        ...
        ggplot(data = trtResults, aes(x = TRTstdBetaMean, y = TRTstdBetaDM)) +
          ...
          ggtitle(paste0("c",cluster," - Treatment"), 
                  subtitle = paste0("mean:",sum(trtResults$TRTsigMean)," DM:",sum(trtResults$TRTsigDM)))
      
        ggsave(paste0(figDir,"/stdBetas_DM_v_mean_Treatment_fdr",as.character(trtFDR),"_c",cluster,".png"))
      }
    }
  }
}
...
```{r are there genes in common across behavioral variables}
...
for (c in c(1:3,5:7)) {
  ...
  DMPBemmreml <- get(paste0("res_full_DMpb_c",cluster))
  eloList <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,15], fdr.level = f)$significant & (flip * DMPBemmreml[,3]) > 0]
  DMPBemmreml <- get(paste0("res_full_aggAsym_DMpb_c",cluster))
  agAsymList <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,15], fdr.level = f)$significant & (flip * DMPBemmreml[,3]) > 0]
  DMPBemmreml <- get(paste0("res_full_aggRec_DMpb_c",cluster))
  agRecList <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,15], fdr.level = f)$significant & (-flip * DMPBemmreml[,3]) > 0]
  DMPBemmreml <- get(paste0("res_full_groom_DMpb_c",cluster))
  groomList <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,15], fdr.level = f)$significant & (flip * DMPBemmreml[,3]) > 0]

  MyVenn <- venneuler(c(A=length(groomList),
                      B=length(agRecList),
                      C=length(agAsymList),
                      D=length(eloList),
                      "A&B"=length(intersect(groomList,agRecList)),
                      "B&C"=length(intersect(agAsymList,agRecList)),
                      "A&C"=length(intersect(agAsymList,groomList)),
                      "A&D"=length(intersect(groomList,eloList)),
                      "B&D"=length(intersect(agRecList,eloList)),
                      "C&D"=length(intersect(agAsymList,eloList)),
                      "A&B&C"=length(intersect(intersect(groomList,agRecList),agAsymList)),
                      "A&B&D"=length(intersect(intersect(groomList,agRecList),eloList)),
                      "A&C&D"=length(intersect(intersect(groomList,agAsymList),eloList)),
                      "B&C&D"=length(intersect(intersect(agAsymList,agRecList),eloList)),
                      "A&B&C&D"=length(intersect(intersect(groomList,agRecList),intersect(eloList,agAsymList)))))
  MyVenn$labels <- c(paste0("groom\n",length(groomList)),
                     paste0("-agRec\n",length(agRecList)),
                     paste0("agAsym\n",length(agAsymList)),
                     paste0("elo\n",length(eloList)))
  #MyVenn$labels <- c("A\n22","B\n7","C\n5","D\n58")
  plot(MyVenn, main = paste0("c",cluster," - ",cellID," - ",betaDir,"-regulated"))
}


### same question - but for CD8, CD4, NK cells and Treatment variable which is col 14

DMPBemmreml <- get(paste0("res_full_DMpb_c","00"))
cd8List <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,14], fdr.level = f)$significant]
DMPBemmreml <- get(paste0("res_full_DMpb_c","14"))
cd4List <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,14], fdr.level = f)$significant]
DMPBemmreml <- get(paste0("res_full_DMpb_c","02"))
nkList <- rownames(DMPBemmreml)[qvalue(p = DMPBemmreml[,14], fdr.level = f)$significant]

MyVenn <- venneuler(c(A=length(cd8List),
                    B=length(cd4List),
                    C=length(nkList),
                    "A&B"=length(intersect(cd8List,cd4List)),
                    "B&C"=length(intersect(nkList,cd4List)),
                    "A&C"=length(intersect(cd8List,nkList)),
                    "A&B&C"=length(intersect(intersect(cd8List,cd4List),nkList))))
MyVenn$labels <- c(paste0("cd8\n",length(cd8List)),
                   paste0("cd4\n",length(cd4List)),
                   paste0("nk\n",length(nkList)))
#MyVenn$labels <- c("A\n22","B\n7","C\n5","D\n58")
plot(MyVenn, main = paste0("treatment fdr 0.20"))


DMPBemmreml <- get(paste0("res_full_DMpb_c","00"))
cd8List <- rownames(DMPBemmreml)
DMPBemmreml <- get(paste0("res_full_DMpb_c","14"))
cd4List <- rownames(DMPBemmreml)
DMPBemmreml <- get(paste0("res_full_DMpb_c","02"))
nkList <- rownames(DMPBemmreml)
...
MyVenn$labels <- c(paste0("cd8\n",length(cd8List)),
                   ...
                   paste0("nk\n",length(nkList)))
#MyVenn$labels <- c("A\n22","B\n7","C\n5","D\n58")
plot(MyVenn, main = paste0("tested genes"))
...
for (cluster in clusterCellID$cluster[-4]) {
    
  #cluster <- "00"
  dir.create(paste0(workDir,"figures/gsea/c",cluster))
  
  for (lmVar in c("trt","behav")) {
    ...
    printOut <- cbind(as.character(GO_res$GO),output$p_values,output$NES_matrix_upreg[,1],output$NES_matrix_downreg[,1])
